...
{
    access_t *access = (access_t *) GC_MALLOC(sizeof(access_t));

    access->order = order;
    access->allow = GC_MALLOC_ATOMIC(sizeof(access_rule_t));
    access->deny = GC_MALLOC_ATOMIC(sizeof(access_rule_t));
...
int access_allow(access_t *access, const char *ip, const char *mask)
{
    ...
    access->allow = (access_rule_t *) GC_REALLOC(access->allow, sizeof(access_rule_t) * (access->nallow + 1));
...
int access_deny(access_t *access, const char *ip, const char *mask)
{
    ...
    access->deny = (access_rule_t *) GC_REALLOC(access->deny, sizeof(access_rule_t) * (access->ndeny + 1));
