        ...
        if (ns >= 0) {
            ...
            if (j == dest->ncur) {
                ...
                if (NAD_NURI_L(src, ns) > 255)
                    uri = malloc(sizeof(char) * (NAD_NURI_L(src, ns) + 1));
                if (NAD_NPREFIX_L(src, ns) > 255)
                    prefix = malloc(sizeof(char) * (NAD_NURI_L(src, ns) + 1));
...
nad_t *nad_deserialize(const char *buf) {
    ...
    if (nad->ecur > 0)
    {
        nad->elems = malloc(sizeof(struct nad_elem_st) * nad->ecur);
    ...
    if (nad->acur > 0)
    {
        nad->attrs = malloc(sizeof(struct nad_attr_st) * nad->acur);
    ...
    if (nad->ncur > 0)
    {
        nad->nss = malloc(sizeof(struct nad_ns_st) * nad->ncur);
    ...
    if (nad->ccur > 0)
    {
        nad->cdata = malloc(sizeof(char) * nad->ccur);
