...
struct _xht
{
    ...
    int count;
    xhn *zen;
    xhn *free_list; // list of zaped elements to be reused.
    int iter_bucket;
    xhn *iter_node;
    int *stat;
};
...
xht *xhash_new(int prime)
{
    ...
    pool_t *p = pool_heap(sizeof(xhn)*prime + sizeof(xht));
    xht *xnew = pnew(p, xht);
    xnew->prime = prime;
    xnew->p = p;
...
void xhash_putx(xht *h, const char *key, int len, void *val)
{
    ...
    {
//        LOG_DEBUG(log, "replacing %s with new val %p", key, val);
...
//    LOG_DEBUG(log, "saving %s val %p", key, val);
...
void *xhash_getx(xht *h, const char *key, int len)
{
    ...
    {
//        LOG_DEBUG(log, "failed lookup of %s", key);
        return NULL;
    }

//    LOG_DEBUG(log, "found %s returning %p", key, n->val);
    return n->val;
}
...
static void xhash_zap_inner(xht *h, xhn *n, int index)
{
    int i = index % h->prime;
...
{
//    LOG_DEBUG(log, "hash free %p", h);
    pool_free(h->p);
}

void xhash_stat( __attribute__ ((unused)) xht *h )
{
#ifdef XHASH_DEBUG
    if ( !h ) return;
