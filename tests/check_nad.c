...
const char *nadtxt[NADTXT_COUNT] = {
...
const char *nadmangled[NADTXT_COUNT] = {
...
"<route xmlns='http://jabberd.jabberstudio.org/ns/component/1.0' from='s2s' to='sm'><presence xmlns='jabber:client' from='anon%majesticmedia.ca@gtalk.jrudevels.org/gmail.E1D35514' to='test@chrome.pl'><status/><show>away</show><priority>0</priority><caps:c xmlns:caps='http://jabber.org/protocol/caps' node='http://mail.google.com/xmpp/client/caps' ext='pmuc-v1 sms-v1 vavinvite-v1' ver='1.1'/><x xmlns='vcard-temp:x:update'><photo>86f065b95e82036afa1eb2180f846e60085f3138</photo></x></presence></route>"
};
...
START_TEST (check_s2s_wrap)
{
    ...
    nad_print(nad, 0, &buf, &len);
    fprintf(stdout, "Mangled packet[%d]:\n%.*s\n", _i, len, buf);
...
"<iq type='set'>\n\
    ...
    ck_assert_int_ge(elem, 0);

    snprintf(spool, sizeof(spool), "%.*s", NAD_ENAME_L(nad, elem), NAD_ENAME(nad, elem));
    spool[sizeof(spool)-1] = '\0';
    ck_assert_str_eq(spool, "options");
