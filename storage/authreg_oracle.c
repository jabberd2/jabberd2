...
static int oracle_ping( authreg_t ar )
{
	Oracle_context_t odpOracleDriver = (Oracle_context_t)ar->private;
	...
	if (nResultCode != 0)
	{
		...
		log_write(ar->c2s->log, LOG_ERR, "storage_oracle.c (oracle_ping): %s", szErrorBuffer);

		// Obtain user configuration
		svHost = config_get_one(ar->c2s->config, "storage.oracle.host", 0);
		svUser = config_get_one(ar->c2s->config, "storage.oracle.user", 0);
		svPass = config_get_one(ar->c2s->config, "storage.oracle.pass", 0);
...
static int _ar_oracle_get_password( authreg_t ar, char *username, char *realm, char password[257] )
{
    Oracle_context_t _ctx = (Oracle_context_t)ar->private;
	char* _sqlbuf = NULL;
	int _nNumberOfFields = 0;
	...
	_nResultCode = checkOCIError(ar, "_ar_oracle_get_user_tuple:define pos", _ctx->ociError, OCIDefineByPos( _ctx->ociStatement, \
									&_ctx->ociDefine, _ctx->ociError, 1, &_password, 257, SQLT_STR, 0, 0, 0, OCI_DEFAULT ) );
	if( _nResultCode != 0 )
	{
		free( _sqlbuf );
		return -1;
	...
	if( strlen(_password)  != 0 )strncpy( password, _password, strlen(_password) );

	return 0;
}

static int _ar_oracle_set_password(authreg_t ar, char *username, char *realm, char password[257])
{
	Oracle_context_t _ctx = (Oracle_context_t)ar->private;
	char* _sqlbuf = NULL;
	int _len = 0;
...
int ar_init(authreg_t ar) {
	...
	log_debug( ZONE, "oracle9i connecting as '%s' to database '%s' on %s:%s", user, dbname, host, port );

#ifdef __COMPATIBLE
	/* Initialize OCI */
	nResultCode = OCIInitialize((ub4) OCI_DEFAULT, (dvoid *)0, (dvoid * (*)(dvoid *, size_t))0, (dvoid * (*)(dvoid *, dvoid *, size_t))0, (void (*)(dvoid *, dvoid *)) 0);

//	fprintf( stdout, "LINE = %d:    nResultCode = %d\n", __LINE__, nResultCode );
	if (nResultCode != 0)
	{
		log_write(ar->c2s->log, LOG_ERR, "(st_oracle_init: ) Could not Initialize OCI (%d)", nResultCode);
		return 1;
	}
...
#else
	nResultCode = OCIEnvCreate( (OCIEnv**)&ociEnvironment, OCI_OBJECT, (dvoid*)0, 0, 0, 0, (size_t)0, (dvoid **)0 );
#endif	
//	fprintf( stdout, "LINE = %d:    nResultCode = %d\n", __LINE__, nResultCode );
	if (nResultCode != 0)
	{
...
//	fprintf( stdout, "LINE = %d:    nResultCode = %d, ociError=%s\n", __LINE__, nResultCode, ociError );
