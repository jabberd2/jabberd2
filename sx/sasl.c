...
void _sx_sasl_open(sx_t *s, Gsasl_session *sd) {
    ...
    /* get the method */
    method = (char *) GC_MALLOC_ATOMIC(sizeof(char) * (strlen(mechname) + 6));
    ...
    if (0 && ctx && ctx->cb) { /* not supported yet */
        ...
        creds.authzid = GC_STRDUP(gsasl_property_fast(sd, GSASL_GSSAPI_DISPLAY_NAME));
        authzid = NULL;
    } else {
        ...
        authzid = (char *) GC_MALLOC_ATOMIC(sizeof(char) * (strlen(creds.authnid) + strlen(realm) + 2));
...
static void _sx_sasl_client_process(sx_t *s, sx_plugin_t *p, Gsasl_session *sd, const char *mech, const char *in, int inlen) {
    ...
    if (mech != NULL) {
        ...
        if (sctx != NULL) GC_FREE(sctx);

        /* allocate and initialize our per session context */
        sctx = (_sx_sasl_sess_t) GC_MALLOC(sizeof(struct _sx_sasl_sess_st));
        sctx->s = s;
        sctx->ctx = ctx;
        ...
        if (ext_id != NULL) {
            ...
            for (i = 0; i < SX_CONN_EXTERNAL_ID_MAX_COUNT; i++)
                if (ext_id[i] != NULL) {
                    ctx->ext_id[i] = GC_STRDUP(ext_id[i]);
                } else {
                    ctx->ext_id[i] = NULL;
                    break;
                }
...
static void _sx_sasl_unload(sx_plugin_t *p) {
    ...
    if (ctx->appname != NULL) GC_FREE(ctx->appname);
    for (i = 0; i < SX_CONN_EXTERNAL_ID_MAX_COUNT; i++)
        if (ctx->ext_id[i] != NULL)
            GC_FREE(ctx->ext_id[i]);
        else
            break;

    if (ctx != NULL) GC_FREE(ctx);
}
...
int sx_sasl_init(__attribute__ ((unused)) sx_env_t *env, sx_plugin_t *p, va_list args) {
    ...
    ctx = (_sx_sasl_t) GC_MALLOC(sizeof(struct _sx_sasl_st));

    ctx->appname = GC_STRDUP(appname);
    ctx->cb = cb;
    ctx->cbarg = cbarg;
    for (i = 0; i < SX_CONN_EXTERNAL_ID_MAX_COUNT; i++)
        ctx->ext_id[i] = NULL;
