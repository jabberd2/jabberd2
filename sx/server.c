...
static void _sx_server_element_start(void *arg, const char *name, const char **atts) {
    ...
    while(attr[0] != NULL) {
        if (!tflag && strcmp(attr[0], "to") == 0) {
            ...
            s->req_to = GC_STRDUP(attr[1]);
            tflag = 1;
        }

        if (!fflag && strcmp(attr[0], "from") == 0) {
            s->req_from = GC_STRDUP(attr[1]);
            fflag = 1;
        }

        if (!vflag && strcmp(attr[0], "version") == 0) {
            s->req_version = GC_STRDUP(attr[1]);
            vflag = 1;
        }
    ...
    /* response attributes */
    if (s->req_to != NULL) s->res_from = GC_STRDUP(s->req_to);
    if (s->req_from != NULL) s->res_to = GC_STRDUP(s->req_from);

    /* Only send 1.0 version if client has indicated a stream version - c/f XMPP 4.4.1 para 4 */
    if (s->req_version != NULL) s->res_version = GC_STRDUP("1.0");
