...
typedef struct sx_env_st        sx_env_t;
typedef struct sx_plugin_st     sx_plugin_t;
...
/** utility: buffer */
typedef struct sx_buf_st sx_buf_t;
struct sx_buf_st {
...
/* make/break */
JABBERD2_API sx_t *sx_new(sx_env_t *env, const char *ip, int port, sx_callback_t cb, void *arg);
...
JABBERD2_API void _sx_buffer_alloc_margin(sx_buf_t *buf, size_t before, size_t after);
...
struct sx_st {
    ...
    unsigned int            rbytes;
    unsigned int            rbytes_total;

    /* read bytes maximum */
    unsigned int            rbytesmax;
...
JABBERD2_API int __sx_event(sx_t *s, sx_event_t e, void *data);
#define _sx_event(s,e,data) __sx_event(s, e, data)
...
#define _sx_state(s,st) do { _sx_debug("%s:%d state change from %d to %d", s->ip, s->port, s->state, st); s->state = st; } while(0)
