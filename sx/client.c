...
static void _sx_client_element_start(void *arg, const char *name, const char **atts) {
    sx_t *s = (sx_t*) arg;
    int tflag = 0, fflag = 0, vflag = 0, iflag = 0;
    size_t i;
    const char **attr;
    sx_error_t sxe;
    ...
    while(attr[0] != NULL) {
        if (!tflag && strcmp(attr[0], "to") == 0) {
            s->res_to = GC_STRDUP(attr[1]);
            tflag = 1;
        }

        if (!fflag && strcmp(attr[0], "from") == 0) {
            s->res_from = GC_STRDUP(attr[1]);
            fflag = 1;
        }

        if (!vflag && strcmp(attr[0], "version") == 0) {
            s->res_version = GC_STRDUP(attr[1]);
            vflag = 1;
        }

        if (!iflag && strcmp(attr[0], "id") == 0) {
            s->id = GC_STRDUP(attr[1]);
            iflag = 1;
        }
    ...
    _sx_debug("stream response: to %s from %s version %s id %s", s->res_to, s->res_from, s->res_version, s->id);
...
static void _sx_client_element_end(void *arg, __attribute__ ((unused)) const char *name) {
    sx_t *s = (sx_t*) arg;
...
static void _sx_client_notify_header(sx_t *s, __attribute__ ((unused)) void *arg) {
    /* expat callbacks */
    ...
    _sx_debug("stream header sent, waiting for reply");
...
void sx_client_init(sx_t *s, unsigned int flags, const char *ns, const char *to, const char *from, const char *version) {
    ...
    _sx_debug("doing client init for sx %s:%d %s", s->ip, s->port, _sx_flags(s));

    s->type = type_CLIENT;
    s->flags = flags;

    if (ns != NULL) s->ns = GC_STRDUP(ns);
    if (to != NULL) s->req_to = GC_STRDUP(to);
    if (from != NULL) s->req_from = GC_STRDUP(from);
    if (version != NULL) s->req_version = GC_STRDUP(version);
    ...
    _sx_debug("stream request: ns %s to %s from %s version %s", ns, to, from, version);
    ...
    _sx_debug("prepared stream header: %.*s", buf->len, buf->data);
