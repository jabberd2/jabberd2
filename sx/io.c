...
void _sx_process_read(sx_t *s, sx_buf_t *buf) {
    ...
    if (s->rbytesmax && s->rbytes > s->rbytesmax) {
        /* parse error */
        _sx_debug("maximum stanza size (%d) exceeded by reading %d bytes", s->rbytesmax, s->rbytes);
    ...
    if (s->state >= state_STREAM)
        while((nad = jqueue_pull(s->rnadq)) != NULL) {
            ...
            _sx_debug("completed nad: %.*s", len, out);
#endif
            ...
            if (NAD_ENS(nad, 0) >= 0 && NAD_NURI_L(nad, NAD_ENS(nad, 0)) == strlen(uri_STREAMS) && strncmp(NAD_NURI(nad, NAD_ENS(nad, 0)), uri_STREAMS, strlen(uri_STREAMS)) == 0 && NAD_ENAME_L(nad, 0) == 5 && strncmp(NAD_ENAME(nad, 0), "error", 5) == 0) {
                ...
                if ((ns = nad_find_scoped_namespace(nad, uri_STREAM_ERR, NULL)) >= 0)
                    if ((elem = nad_find_elem(nad, 0, ns, "text", 1)) >= 0)
                        if (NAD_CDATA_L(nad, elem) > 0) {
                            errstring = (char *) GC_MALLOC_ATOMIC(sizeof(char) * (NAD_CDATA_L(nad, elem) + 1));
                ...
                if (errstring == NULL && NAD_CDATA_L(nad, 0) > 0) {
                    errstring = (char *) GC_MALLOC_ATOMIC(sizeof(char) * (NAD_CDATA_L(nad, 0) + 1));
                    sprintf(errstring, "%.*s", NAD_CDATA_L(nad, 0), NAD_CDATA(nad, 0));
                }

                /* if not available, log the whole packet for debugging */
                if (errstring == NULL) {
                    char *xml; unsigned int xlen;
                    nad_print(nad, 0, &xml, &xlen);
                    errstring = (char *) GC_MALLOC_ATOMIC(sizeof(char) * (xlen + 1));
                ...
                if (errstring != NULL) GC_FREE(errstring);
            ...
            if ((s->flags & SX_WEBSOCKET_WRAPPER) && NAD_ENS(nad, 0) >= 0 && NAD_NURI_L(nad, NAD_ENS(nad, 0)) == strlen(uri_XFRAMING) && strncmp(NAD_NURI(nad, NAD_ENS(nad, 0)), uri_XFRAMING, strlen(uri_XFRAMING)) == 0 && NAD_ENAME_L(nad, 0) == 5 && strncmp(NAD_ENAME(nad, 0), "close", 5) == 0) {
                _sx_debug("<close/> frame @ depth %d", s->depth);
                s->fail = 1;
                break;
            }
...
int sx_can_read(sx_t *s) {
    ...
    _sx_debug("%s:%d ready for reading", s->ip, s->port);
    ...
    if (read == 0) {
        ...
        _sx_debug("decoded 0 bytes read data - this should not happen");
        _sx_buffer_free(in);

    } else {
        _sx_debug("passed %d read bytes", in->len);
        ...
        _sx_debug("decoded read data (%d bytes): %.*s", out->len, out->len, out->data);
...
static int _sx_get_pending_write(sx_t *s) {
    ...
    _sx_debug("encoding %d bytes for writing: %.*s", in->len, in->len, in->data);
...
int sx_can_write(sx_t *s) {
    ...
    _sx_debug("%s:%d ready for writing", s->ip, s->port);

    ret = _sx_get_pending_write(s);
    if (ret < 0) {
        /* fatal error */
        _sx_debug("fatal error after attempt to write on fd %s:%d", s->ip, s->port);
    ...
    _sx_debug("handing app %d bytes to write", out->len);
    ...
    if (written < 0) {
        ...
        s->want_write = 0;
        return 0;
    } else if ((unsigned)written < out->len) {
...
int _sx_nad_write(sx_t *s, nad_t *nad, int elem) {
    ...
    if (s->state >= state_CLOSING) {
        _sx_debug("stream closed, dropping outgoing packet");
        nad_free(nad);
        return 1;
    }
    ...
    _sx_debug("queueing for write: %.*s", len, out);
...
int _sx_raw_write(sx_t *s, const char *buf, int len) {
    ...
    if (s->state >= state_CLOSING) {
        _sx_debug("stream closed, dropping outgoing raw data");
        return 1;
    }

    _sx_debug("queuing for write: %.*s", len, buf);
