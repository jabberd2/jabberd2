...
sx_t *sx_new(sx_env_t *env, const char *ip, int port, sx_callback_t cb, void *arg) {
    ...
    if (env != NULL) {
        s->plugin_data = calloc(1, sizeof(void *) * env->nplugins);
...
void sx_free(sx_t *s) {
    ...
    free((void*)s->req_to);
    free((void*)s->req_from);
    free((void*)s->req_version);

    free((void*)s->res_to);
    free((void*)s->res_from);
    free((void*)s->res_version);
...
void _sx_reset(sx_t *s) {
    ...
    new = malloc(sizeof(sx_t));
    memcpy(new, s, sizeof(sx_t));
    sx_free(new);
...
sx_buf_t *_sx_buffer_new(const char *data, int len, _sx_notify_t notify, void *notify_arg) {
    ...
    if (len <= 0) {
        ...
        buf->data = buf->heap = malloc(sizeof(char) * len);
        if (data != NULL)
